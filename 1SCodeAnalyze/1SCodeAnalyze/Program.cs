...
namespace _1SCodeAnalyze
{
    class Program
    {
        ...
        {
			Console.Write("Программа для синтаксического анализа кода конфигурации 1С для работы необходимо указать путь к выгруженным файлам конфигурации\nИначе будут проверены все локальные файлы\n");
		...
		private static void ПолучитьФайлыРекурсивно(DirectoryInfo dir, List<FileInfo> files, string arg1)
        {
			...
			if (file.Extension.ToUpper ().Contains ("BSL") || file.Extension == "" || file.Extension == arg1||file.Extension == ("."+arg1)) {
